...
namespace moveit_opw_kinematics_plugin {
  ...
  public:
    ...
    virtual bool
    searchPositionIK(const geometry_msgs::msg::Pose &ik_pose, const std::vector<double> &ik_seed_state,
                     double timeout,
                     std::vector<double> &solution, moveit_msgs::msg::MoveItErrorCodes &error_code,
                     const kinematics::KinematicsQueryOptions &options = kinematics::KinematicsQueryOptions())
    const override;
    ...
    rcl_interfaces::srv::GetParameters::Response::SharedPtr getParamsFromNode(
      const std::string &node_name, const std::string &parameter_name);
    ...
    virtual bool
    getPositionIK(const std::vector<geometry_msgs::msg::Pose> &ik_poses, const std::vector<double> &ik_seed_state,
  ...
  private:
    ...
    //        bool timedOut(const rclcpp::Clock &start_time, double duration) const;
    ...
    bool assignParameters(const std::map<std::string, double> &geom_params,
                          const std::vector<double> &joint_offsets,
                          const std::vector<int64_t> &joint_sign_corrections);


    // Cache for parameters fetched from move_group
    std::map<std::string, rcl_interfaces::msg::ParameterValue> cached_parameters_;

    template<typename T>
    bool lookupParam(const rclcpp::Node::SharedPtr &node, const std::string &param, T &val, const T &default_val);

    template<typename T>
    bool convertParameterValue(const rcl_interfaces::msg::ParameterValue &param_value, T &val);

    template<typename T>
    bool lookupParamFromMoveGroupAsync(const rclcpp::Node::SharedPtr &node, const std::string &param, T &val,
                                       const T &default_val);

    template<typename T>
    bool lookupCachedParam(const std::string &param, T &val, const T &default_val);

    bool cacheParametersFromMoveGroup();

    template<typename T>
    bool lookupParamLocal(const rclcpp::Node::SharedPtr &node, const std::string &param, T &val, const T &default_val);
